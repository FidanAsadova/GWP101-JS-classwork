...
users.map(
  (el) => el.surname[0] === "R" && el.uni === "UNEC" ? console.log(el) :  null
);

// 2.2.Show users whose age is eqaul to 22 and name start 'a'.

users.map(
    (elm) => elm.name[0] === "A" && elm.age === "22" ? console.log(elm) :  null
  );

  // 2.3.Return new array, if user is male add in front of name 'Mr', or Ms. (e.g. name:'Mr Anar', 'Ms Aynure').

let u1 = users.map((elmt)=> elmt.gender === "male" ? elmt.name = `Mr${elmt.name}` :elmt.name = `Mrs${elmt.name}` )
console.log(u1);
